...
router.post("/update-product", protect, (req, res) => {
  const data = req.body;
  const textWithAppostrophe = data.description.replace("'", "''");
  console.log(textWithAppostrophe);
  console.log(data.description);
  const query = `UPDATE MsProduct SET ProductName = '${data.name}', ProductDescription = '${textWithAppostrophe}', ProductCategory = '${data.category}'
