...
//CHECK IF USER EVER SIGN IN TO GOOGLE
router.route("/hasGoogleLogin").post((req, res) => {
  var email = req.body.email;
  var sqlQuery = `SELECT * FROM mscustomer WHERE CustomerEmail LIKE '${email}';`
  con.query(
    sqlQuery,
    function (error, data, fields) {
      console.log(data);
      if (error || data.length === 0) {
        // if error send response here
        res.send(JSON.stringify({ success: false, message: "user has not sign in" }));
      } else {
        // if success send response here
        res.send(JSON.stringify({ success: true, user: data, message: "success" }));
      }
    }
  );
})
